...
class DetailedAnalysisTests(unittest.TestCase):
    ...
    def test_regression_metrics_are_cross_validated_and_consistent(self) -> None:
        ...
        self.assertIn("5-fold", regression.notes[2])
    ...
    def test_excel_exports_have_charts_formulas_and_method_sheets(self) -> None:
        ...
        self.assertEqual(
            ...
            ["Summary", "Coefficients", "Predictions", "Method"],
        )
        self.assertGreaterEqual(len(regression["Summary"]._charts), 1)
        self.assertGreaterEqual(len(regression["Coefficients"]._charts), 1)
        self.assertTrue(str(regression["Summary"]["C12"].value).startswith("="))
        self.assertTrue(str(regression["Predictions"]["F2"].value).startswith("="))
        self.assertEqual(regression["Predictions"]["D2"].alignment.horizontal, "center")

        clustering = load_workbook(clustering_path, data_only=False)
        self.assertEqual(
            clustering.sheetnames,
            ["Summary", "Cluster Profiles", "K Selection", "ZIP Assignments", "Method"],
        )
        self.assertGreaterEqual(len(clustering["Cluster Profiles"]._charts), 1)
        self.assertGreaterEqual(len(clustering["K Selection"]._charts), 1)
        self.assertEqual(clustering["ZIP Assignments"]["J2"].alignment.horizontal, "center")
